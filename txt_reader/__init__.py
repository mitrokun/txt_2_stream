...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    if DOMAIN not in hass.data:
        ...
        async def handle_play(call: ServiceCall):
            ...
            file_path, manual_idx = call.data["file_path"], call.data.get("block_index")
            ...
            for sid in list(sessions.keys()):
                if now - sessions[sid].get("last_accessed", now) > 43200: sessions.pop(sid, None)

            session_id = uuid.uuid4().hex
            sessions[session_id] = {
                "config": config, "file_path": file_path, "chunks": chunks,
                "store": global_store, "start_index": start_index, "last_accessed": now
            }
